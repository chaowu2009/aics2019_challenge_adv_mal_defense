...
def PRelu(x, name='PRelu'):
    with tf.variable_scope(name):
        alpha = tf.get_variable('ALPHA_PRelu',shape=x.get_shape()[1:],dtype=tf.float32,initializer=tf.zeros_initializer(),trainable=True)
        pos = tf.nn.relu(x)
...
def read_tarfile(file_path, file_ext = '.json'):
    import tarfile
    if not os.path.exists(file_path) and not tarfile.is_tarfile(file_path):
        raise IOError()
    try:
        ...
        for m in tar:
            f = tar.extractfile(m)
            if f and file_ext == os.path.splitext(m.name)[-1]:
                content.append(f)
        return content
    except OSError:
        raise OSError
